...
/**
 * Vite dev server plugin for sebuf API routes — disabled in Orb fork.
 * The original plugin booted server/ handlers for local /api/{domain}/v1/*
 * routing. server/ was removed; production keeps Vercel's serverless api/.
 */
function sebufApiPlugin(): Plugin {
  return { name: 'sebuf-api-noop' };
}
