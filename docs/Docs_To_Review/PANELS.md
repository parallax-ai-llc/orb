...
| `orb-variant` | Last-active variant (triggers reset on change) | `App.ts` |
...
| Active variant | `orb-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`orb-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:
