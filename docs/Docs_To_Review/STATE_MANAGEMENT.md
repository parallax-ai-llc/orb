...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('orb-theme', 'light')
...
|-----|---------|--------|--------|
| `orb-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `orb-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
