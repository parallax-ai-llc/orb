...
On the desktop app, the variant is stored in `localStorage['orb-variant']` and can be switched without rebuilding. The variant selector in the header bar navigates between deployed domains on the web or toggles the localStorage value on desktop.
