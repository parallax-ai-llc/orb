...
export class App {
  ...
  constructor(containerId: string) {
    ...
    const storedVariant = localStorage.getItem('orb-variant');
    ...
    if (storedVariant !== currentVariant) {
      ...
      localStorage.setItem('orb-variant', currentVariant);
      ...
      // Orb fork — re-seed panel settings on every variant switch so each
      // variant (full / tech / finance / commodity / energy) shows its own
      // panel set. Without this, the first variant the user lands on wins
      // forever because the one-shot migration only runs once per browser.
      const SEED_VARIANT_KEY = 'orb-last-seeded-variant';
      const lastSeededVariant = localStorage.getItem(SEED_VARIANT_KEY);
      if (lastSeededVariant !== SITE_VARIANT) {
        ...
        localStorage.setItem(SEED_VARIANT_KEY, SITE_VARIANT);
      }
