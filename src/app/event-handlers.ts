...
export class EventHandlerManager implements AppModule {
  ...
  private setupEventListeners(): void {
    ...
    // Orb theater-bar search input — proxies focus/click to the modal search.
    const theaterSearch = document.getElementById('orbTheaterSearch') as HTMLInputElement | null;
    theaterSearch?.addEventListener('mousedown', (e) => {
      e.preventDefault();
      theaterSearch.blur();
      track('search-open', { source: 'theater-bar' });
      openSearch();
    });
    theaterSearch?.addEventListener('focus', () => {
      theaterSearch.blur();
      track('search-open', { source: 'theater-bar' });
      openSearch();
    });
    // Orb alert/bell button — proxies to the existing Intelligence Findings
    // badge (mounted in .header-right by IntelligenceFindingsBadge.mount()).
    // The original badge button is visually hidden via CSS; the bell takes
    // its place. After dispatching click on the badge to toggle the dropdown,
    // reposition the dropdown with fixed positioning anchored to the bell
    // so it appears below the bell instead of the (hidden, off-position)
    // badge.
    document.getElementById('orbAlertBtn')?.addEventListener('click', (e) => {
      e.stopPropagation();
      const bell = e.currentTarget as HTMLElement;
      const intelBadge = document.querySelector<HTMLButtonElement>('.intel-findings-badge');
      if (intelBadge) {
        intelBadge.click();
      }
    ...
    // (iranAttacks, hotspots, conflicts, bases, nuclear, etc.).
    const orbLayerList = document.getElementById('orbLayerList');
    // Sync initial .on state from current mapLayers snapshot so the
    // rail matches the underlying map on first paint.
    if (orbLayerList) {
      orbLayerList.querySelectorAll<HTMLElement>('.pxm-layer').forEach((el) => {
        const key = el.dataset.layer as keyof MapLayers | undefined;
        if (key && this.ctx.mapLayers[key]) el.classList.add('on');
      });
    }
    orbLayerList?.addEventListener('click', (e) => {
      const row = (e.target as Element).closest<HTMLElement>('.pxm-layer');
      if (!row) return;
      const key = row.dataset.layer as keyof MapLayers | undefined;
      if (!key) return;
      const nextOn = !this.ctx.mapLayers[key];
    ...
    // Orb THEATER tabs → site variant switch. Mirrors the original
    // variant-switcher click handler: localhost/desktop uses localStorage
    // + reload; production navigates to the variant subdomain.
    const theaterBar = document.getElementById('orbTheaterBar');
    theaterBar?.addEventListener('click', (e) => {
      const tab = (e.target as Element).closest<HTMLElement>('.pxm-tab');
      if (!tab) return;
      const variant = tab.dataset.variant;
      if (!variant || variant === SITE_VARIANT) return;
      const isLocalDev = this.ctx.isDesktopApp
  ...
  setupAuthWidget(): void {
    // Auth widget permanently removed in this fork — no modal, no header widget.
    return;
  }
