...
export class PanelLayoutManager implements AppModule {
  ...
  private async fetchGitHubStars(): Promise<void> {
    try {
      const response = await fetch('https://github.com/parallax-ai-llc/orb');
  ...
  async renderLayout(): Promise<void> {
    this.ctx.container.innerHTML = `
      ...
      <div class="header">
        ...
        return `
          ...
          <div class="orb-brand-block">
            <span class="logo">ORB</span><span class="logo-mobile">Orb</span>
          </div>
          ...
          <a href="https://github.com/parallax-ai-llc/orb" target="_blank" rel="noopener" class="github-link" title="${t('header.viewOnGitHub')}">
      ...
      <div class="pxm-theater-bar" role="tablist" aria-label="Theater" id="orbTheaterBar">
        ${(() => {
          // Theater tabs ↔ site variants. Clicking switches the variant via
          // localStorage + reload on localhost/desktop, or navigates to the
          // variant subdomain in production.
          const variants = [
            { variant: 'full',      label: 'Global' },
            { variant: 'tech',      label: 'Tech' },
            { variant: 'finance',   label: 'Finance' },
            { variant: 'commodity', label: 'Commodity' },
            { variant: 'energy',    label: 'Energy' },
          ];
          return variants.map(v => `
            <div class="pxm-tab${v.variant === SITE_VARIANT ? ' is-on' : ''}" role="tab" data-variant="${v.variant}" aria-selected="${v.variant === SITE_VARIANT ? 'true' : 'false'}">
              <span class="pxm-tab-dot"></span>
              <span>${v.label}</span>
            </div>
          `).join('');
        })()}
        <div class="pxm-theater-spacer"></div>
        <div class="pxm-theater-query">
          <span class="pxm-theater-query-label">QUERY</span>
          <input type="search" class="pxm-theater-search" id="orbTheaterSearch" placeholder="// search intel · ops · feeds · regions" autocomplete="off" />
        </div>
      </div>
      ...
      <nav class="mobile-menu" id="mobileMenu">
        ...
        <a class="mobile-menu-item" href="https://www.instagram.com/imnothansanghoon/" target="_blank" rel="noopener">
          ...
          <span class="mobile-menu-item-label">@sanghoon</span>
        </a>
        <div class="mobile-menu-divider"></div>
  ...
  private async createPanels(): Promise<void> {
    ...
    // stock-analysis and stock-backtest removed in Orb fork (Pro-only features).
    ...
    // latest-brief removed in Orb fork (Pro-only feature).
    ...
    // trade-policy removed in Orb fork (Pro-only feature).
    ...
    // DeductionPanel and RegionalIntelligenceBoard removed in Orb fork
    // (Pro-only features dropped from this build).
    ...
    // wsb-ticker-scanner removed in Orb fork (Pro-only feature).
    ...
    // daily-market-brief, market-implications, chat-analyst removed in Orb fork (Pro-only features).
