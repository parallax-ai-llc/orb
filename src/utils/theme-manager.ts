 ...
 */
// Orb fork — theme is permanently dark. Helpers return 'dark' regardless of
// stored prefs so the UI cannot fall back to light mode.
...
export function applyStoredTheme(): void {
  // Orb fork — always dark.
  document.documentElement.dataset.theme = 'dark';
  try { localStorage.setItem(STORAGE_KEY, 'dark'); } catch { /* noop */ }
  updateThemeMetaColor('dark', document.documentElement.dataset.variant);
}
