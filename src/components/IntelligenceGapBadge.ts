...
const DEFAULT_ALERT_HOURS = 6;
const TIME_WINDOW_STORAGE_KEY = 'orb-time-window-hours';
const STORAGE_KEY = 'worldmonitor-intel-findings';
const POPUP_STORAGE_KEY = 'wm-alert-popup-enabled';

function getAlertHours(): number {
  const raw = localStorage.getItem(TIME_WINDOW_STORAGE_KEY);
  const parsed = raw ? Number(raw) : NaN;
  return Number.isFinite(parsed) && parsed > 0 ? parsed : DEFAULT_ALERT_HOURS;
}
...
export class IntelligenceFindingsBadge {
  ...
  private mergeFindings(): UnifiedFinding[] {
    ...
    const alerts = getRecentAlerts(getAlertHours());
