...
export class Panel {
  ...
  constructor(options: PanelOptions) {
    ...
    // Orb tactical ID suffix — deterministic 3-digit code per panel key.
    const idTag = document.createElement('span');
    idTag.className = 'panel-id-tag';
    let _idHash = 0;
    for (let i = 0; i < options.id.length; i++) _idHash = (_idHash * 31 + options.id.charCodeAt(i)) % 1000;
    idTag.textContent = `PNL-${String(_idHash).padStart(3, '0')}`;
  ...
  public showLocked(features: string[] = []): void {
    ...
    // Upgrade CTA removed in this fork — panel just shows locked state without an upgrade path.
  ...
  public showGatedCta(reason: PanelGateReason, _onAction: () => void): void {
    ...
    // Upgrade/Sign-in CTA button removed in this fork.
